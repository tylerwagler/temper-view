...
export const LlamaCppCard: React.FC<LlamaCppCardProps> = ({ stats, onHide, isAdmin, session }) => {
    ...
    React.useEffect(() => {
        if (modelName && availableModels) {
            // Normalize strings for matching (remove spaces, dashes, underscores, lowercase)
            const normalizeStr = (s: string) => s.toLowerCase().replace(/[\s\-_]/g, '');
            const normalizedModelName = normalizeStr(modelName);

            const loadedModel = availableModels.find(m => {
                const normalizedName = normalizeStr(m.name);
                // Check if model name appears in the GGUF filename
                return normalizedModelName.includes(normalizedName);
            });
    ...
    const startMutation = useMutation({
        mutationFn: async (modelOverride?: string) => {
            setIsStarting(true);
            ...
            console.log('[LlamaCppCard] Switching to:', targetModel);

            // Switch to selected model first
            await switchModel(targetModel, token);
    ...
    return (
        <div className="bg-dark-800 rounded-lg p-4 border border-dark-700 h-full min-w-[300px] max-w-[300px] relative group overflow-hidden flex flex-col">
            ...
            <div className="flex flex-col mb-2 pr-6 flex-none border-b border-dark-700 pb-3">
                {/* ... (Status indicator remains same) ... */}
                <div className="flex items-center justify-between mb-2">
                    <h3 className="text-lg font-semibold text-white">AI</h3>
                    <div className="inline-flex items-center gap-1">
                        <span className={`inline-block w-2 h-2 rounded-full ${stats?.status === 'error' ? 'bg-red-500' :
                            stats?.status === 'loading' ? 'bg-yellow-500 animate-pulse' :
                                stats?.status === 'ready' ? 'bg-green-500' :
                                    'bg-dark-600'
                            }`}></span>
                        <span className={`text-xs ${stats?.status === 'error' ? 'text-red-400' :
                ...
                {isAdmin && (
                    <div className="space-y-2">
                        <div className="flex gap-1.5">
                            ...
                            <button
                                onClick={() => startMutation.mutate(selectedModel)}
